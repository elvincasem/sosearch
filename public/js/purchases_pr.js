...
function saveunitprice(pritemsid){
	var pritemqty = document.getElementById("pritemqty-"+pritemsid).value;
	var pritemunit = document.getElementById("pritemunit-"+pritemsid).value;
	var unitprice = document.getElementById("unitprice-"+pritemsid).value;
	var availability = document.getElementById("availability-"+pritemsid).value;
	//var availability = document.getElementById("availability-"+apritemsid).value;
	//alert(unitprice);
	$.ajax({
		url: '../updateunitprice',
		type: 'post',
		data: {pritemsid: pritemsid,unitprice:unitprice,availability:availability,pritemqty:pritemqty,pritemunit:pritemunit},
		success: function(response) {
			console.log(response);
			//location.reload();
			$('#general-table').load(document.URL +  ' #general-table');
	...
	$.ajax({
		...
		success: function(response) {
			...
			for(var ctr=0;ctr<data.length; ctr++){
				...
				var availability = document.getElementById("availability-"+data[ctr].pritemsid).value;
				...
				url: '../updateallprices',
				type: 'post',
				data: {pritemsid: pritems_id, unitprice:unitprice,availability:availability},
				success: function(response) {
						console.log(response);
				}
				});
...
function addpo(){
			...
			var prid = document.getElementById("aprid").value;
...
function addapr(){
			...
			$.ajax({
				...
				data: {aprno:aprno},
				success: function(response) {
					var duplicateapr = response;
					if(duplicateapr>=1){
						alert("Duplicate APR No.")
					}else{
						$.ajax({
						url: '../addapr',
						type: 'post',
						data: {prid:prid,aprdate: aprdate,aprno:aprno,modeofprocurement:modeofprocurement},
						success: function(response) {
							
							//var lastid = JSON.parse(response);
							
							//alert(response);
							window.location.href = "../../apr/details/"+response;
						}
						});
