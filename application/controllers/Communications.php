...
class Communications extends CI_Controller
{
	...
	public function index()
	{
		...
		header('Location: communications/filter/'.$currenty.'/'.$currentm);
	...
	public function filter($year,$month){
		...
		$data['employeelist'] = json_encode($this->socharts_model->employeelist());
		$data['commlist'] = json_encode($this->socharts_model->commlist($year,$month));
		$data['commlisttable'] = $this->socharts_model->commlisttable($year,$month);
		//$data['totalheiperregion_table'] = $this->socharts_model->totalheiperregion_table();
		//print_r($data['totalheiperregion']);
		$this->load->view('charts/communications_view',$data);
		$this->load->view('inc/footer_view',$js);
	}
