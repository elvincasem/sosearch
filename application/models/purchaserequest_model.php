...
class Purchaserequest_model extends CI_Model
{
	...
	public function get_pr_items($id)
	{
	...
	{
		$itemlist = $this->db->query("SELECT * from purchase_pr_items where prid='$id' AND availability='NO'");
		return $itemlist->result_array();
		
		
	}
	
	public function get_list_items_canvass($id)
	{
		$itemlist = $this->db->query("SELECT * from purchase_pr_items where prid='$id' AND availability='NO'");
		return $itemlist->result_array();
	...
	public function updateunitprice($pritemsid,$unitprice,$availability,$pritemqty,$pritemunit)
	{
				
		$sql = "update purchase_pr_items set unitprice=".$this->db->escape($unitprice).",availability=".$this->db->escape($availability).",qty=".$this->db->escape($pritemqty).",unit=".$this->db->escape($pritemunit)." where pritemsid=".$this->db->escape($pritemsid)."";
	...
	public function updateallprices($pritemsid,$unitprice,$availability)
	{
				
		$sql = "update purchase_pr_items set unitprice=".$this->db->escape($unitprice).",availability=".$this->db->escape($availability)." where pritemsid=".$this->db->escape($pritemsid)."";
	...
	public function saveapr($aprdate,$aprno,$prid,$modeofprocurement)
	{
		$this->load->library('session');
		$this->session;
		$uid = $this->session->userdata('uid');
		
		$sql = "INSERT INTO purchase_apr (aprdate,aprno,addedby,prid) VALUES (".$this->db->escape($aprdate).", ".$this->db->escape($aprno).", ".$this->db->escape($uid).", ".$this->db->escape($prid).")";
		$this->db->query($sql);
		...
		//update current pr aprno
		$sql2 = "UPDATE purchase_pr set aprno=".$this->db->escape($aprno)." where prid='$prid'";
		$this->db->query($sql2);
		
		//copy values from other tables
		$sqlselect2 = $this->db->query("SELECT $currentid as aprid,purchase_pr_items.itemid as itemid,purchase_pr_items.description,purchase_pr_items.qty,purchase_pr_items.unit,purchase_pr_items.unitprice,purchase_pr_items.availability FROM purchase_pr_items WHERE purchase_pr_items.prid='$prid' AND availability='YES'");
		$result_array =  $sqlselect2->result_array();
		$this->db->insert_batch('purchase_apr_items',$result_array); 
	...
	public function checkduplicateapr($aprno)
	{
		
		$sql = $this->db->query("SELECT COUNT(*) AS duplicateaprno FROM purchase_apr WHERE aprno='$aprno'");
		$aprmain = $sql->result_array();
		echo $aprmain[0]['duplicateaprno'];
	}
	...
	public function getrelatedapr($prid)
	{
		$relatedapr = $this->db->query("SELECT aprid,aprno from purchase_apr where prid='$prid'");
		$prmain = $relatedapr->result_array();
		if($prmain == null){
			return 0;
		}else{
			return $prmain[0];
